 ...
 * @class
 */
class ReadTextRegistersResponseBody extends ModbusResponseBody {
   ...
   */
  static fromRequest (requestBody, textRegisters) {
    const startByte = requestBody.start * 2
    const endByte = startByte + (requestBody.count * 2)

    const buf = textRegisters.slice(startByte, endByte)

    return new ReadTextRegistersResponseBody(buf.length, buf)
  }
  ...
  static fromBuffer (buffer) {
    ...
    return new ReadTextRegistersResponseBody(byteCount, values, payload)
  }
...
module.exports = ReadTextRegistersResponseBody
